  ...
  created() {
    ...
    this.sendGETRequest("/layout/").then((layout_data) => {
      this.sendGETRequest("/widgets/").then((widgets_data) => {
        let widgets = widgets_data['_embedded']['jSONObjectList'];
        this.layout = layout_data['_embedded']['jSONObjectList'];
        for (const item of widgets) {
          if (!this.layout.some((e) => e.item === item.item && e.id === item.id)) {
      ...
      if(this.selectedItems ==  null) {
        ...
        for(const selectedItem of this.selectedItems) {
          ...
          if(this.lists[nextIndex].name == "home_layout") {
            this.addLayoutItem(item.widgetType, item.id);
          } else if (this.lists[nextIndex].name == "available_widgets") {
            this.removeLayoutItem(item.dashboardId);
          }
        }
        this.selectedItems = [];
