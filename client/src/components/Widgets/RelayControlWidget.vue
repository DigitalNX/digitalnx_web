            ...
            <div class="relay-control">
                <div v-bind:key="relay.id" v-for="relay in relays.slice(0, number_of_relays)" class="relay">
                    ...
                    <template v-if="relay.controlled_by_scheduler">
                      <a href="#" @click="setSelectedRelayId(relay.id)" data-bs-toggle="modal" data-bs-target="#RelaySchedulerModal"><b class="relay-text" style="color: green"> {{ relay.deviceName}}</b></a>
                    </template>
                    <template v-else>
                      <a href="#" @click="setSelectedRelayId(relay.id)" data-bs-toggle="modal" data-bs-target="#RelaySchedulerModal"><span class="relay-text"> {{ relay.deviceName}}</span></a>
                    </template>
